...
try:
    from js import runMinimap2
    from pyodide.ffi import to_js
    # pyodide context
    async def run_minimap2(args):
        js_args = to_js(args)
        jsProxy = await runMinimap2(js_args)
    ...
    async def run_minimap2(args):
        out = subprocess.check_output(['minimap2'] + args)
        return out.decode()
...
async def main():
    args = get_arguments()
    await extract_reads(args.reads, args.start, args.end, args.neg, args.threads,
                  args.min_id, args.max_gap, args.neg_id, args.neg_gap)


async def extract_reads(reads, start, end, neg, threads, min_id, max_gap, neg_id, neg_gap):
    """
    This function contains the tool's main functionality. It's factored out of the main function
    for easier testing.
    """
    start_alignments = await align_reads(reads, start, threads, min_id, max_gap)
    end_alignments = {} if end is None else await align_reads(reads, end, threads, min_id, max_gap)
    neg_alignments = {} if neg is None else await align_reads(reads, neg, threads, neg_id, neg_gap, True)
    ...
    for name, header, seq, qual in iterate_fastq(str(reads)):
        total += 1
...
async def align_reads(reads, target, threads, min_id, max_gap, negative=False):
    print(f'\nAligning {reads} to {target}:', file=sys.stderr)

    args = ['-c', '-x', 'map-ont', '-t', str(threads),
                        str(target), str(reads)]
    out = await run_minimap2(args)
    alignments = [Alignment(x) for x in out.splitlines()]
